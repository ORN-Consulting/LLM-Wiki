...
- `light` — 스크립트 결과 + 요약 인덱스만 읽고 보고. **매 ingest 후 자동 실행** (INGEST 11단계). 토큰 소모 0.
- `standard` — 후보 페이지 10~20개를 실제 비교. 기본값. **주 1회, 또는 소스 5개 이상 ingest 후** 수동 실행 권장.
- `deep` — 전수 검사, 캐시 무시. **월 1회**, 또는 구조 개편·페이지 이동 후, 외부 공유 전 품질 점검 시 수동 실행.
